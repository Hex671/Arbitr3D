...
class Conv3x3Norm(torch.nn.Module):
    ...
    def forward(self, input, **kwargs):
        # print(f">>> Conv3x3Norm input:{input.device}")
...
class DyConv(torch.nn.Module):
    ...
    def forward(self, inputs):
        # print(">>> DyConv inputs device:", inputs["visual"][0].device)
...
class VLDyHead(torch.nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE == "bert-base-uncased":
            local_model_path = "/data/xhhuang/project/hf_models/bert-base-uncased"
            lang_cfg = BertConfig.from_pretrained(local_model_path)
